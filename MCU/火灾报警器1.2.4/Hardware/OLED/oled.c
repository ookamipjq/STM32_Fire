...
void OLED_ShowBNum(u8 x,u8 y,float num,u8 len,u8 size2)//len:´øÐ¡Êýµã¡¢¸ººÅÊý¾Ý¸öÊý
{         	
	u8 t,temp,i=0;
	u8 enshow=0;
	u16 k;
	...
	k=num*10; //´Ë´¦ÎªÏÔÊ¾Ò»Î»Ð¡Êý*10×ª»¯ÎªÕûÊý£¬ÈôÒªÏÔÊ¾¼¸Î»¾Í@ÎÒ£¨×Ô¼ºÏë°ì·¨£©
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
			if(((k/oled_pow(10,len-t-2)%10)!=0)&&(i==1))//ÅÐ¶ÏÊÇ·ñÎª¸ºÊýÇÒÔÚ×î¸ßÎ»Ç°Ò»Î»
			{
			OLED_ShowChar(x+(size2/2)*t,y,'-',size2);
			i=0;	                                    //Çå³ýÅÐ¶ÏºóÒ»Î»µÄ±êÖ¾
			}
			else
		    OLED_ShowChar(x+(size2/2)*t,y,' ',size2);   //Èç¹ûÃ»µ½Êý×Ö¾ÍÏÔÊ¾¿Õ¸ñ
			continue;
			}else enshow=1;		//´Ë´¦ÊÇÅÐ¶ÏÊÇ·ñÒªÏÔÊ¾Êý×Ö	
		}
		if(t==len-1)
		{
		OLED_ShowChar(x+(size2/2)*t,y,'.',size2);
       //ÅÐ¶ÏÊÇ·ñÎª×îºóÒ»Î»µÄÇ°Ò»Î»£¨ÏÔÊ¾Ò»Î»Ð¡Êý£©
		t++;
	 	...
	 	OLED_ShowChar(x+(size2/2)*t,y,temp+'0',size2); //Ò»Î»Ò»Î»ÏÔÊ¾ÏÂÈ¥
	 }
}
